...
export class TestLogger {
	...
	static logTestStart(testName: string, description: string): void {
		console.log(
			`\n🚀 [${TestLogger.getTimestamp()}] Starting Test: ${testName}`,
		);
	...
	static logTestEnd(testName: string, success: boolean): void {
		...
		console.log("─".repeat(80));
		console.log(
			`🏁 [${TestLogger.getTimestamp()}] Test Completed: ${testName} - ${status}\n`,
		);
	}
...
export class DataUtils {
	...
	static generateRandomEmail(domain: string = "example.com"): string {
		const username = DataUtils.generateRandomString(8, true).toLowerCase();
...
export class AssertionUtils {
	...
	static async assertTextContent(
		...
		TestLogger.logValidation(
			`${elementName} has correct text: "${expectedText}"`,
			true,
		);
	}
	...
	static async assertElementCount(
		...
		TestLogger.logValidation(
			`${elementName} count is correct: ${expectedCount}`,
			true,
		);
	}
}
...
export class WaitUtils {
	...
	static async waitForAttributeValue(
		...
		TestLogger.logAction(
			`Waiting for ${selector} to have ${attribute}="${value}"`,
		);
