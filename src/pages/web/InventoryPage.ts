...
import { ProductData } from '@data/testdata/saucedemo.products';
...
export class InventoryPage {
  ...
  /**
   * Get a product container by product ID
   * @param productId - The product identifier
   */
  private getProductContainer(productId: string): Locator {
    // Use the actual SauceDemo structure - find inventory item containing the product
    return this.page.locator(`[data-test="inventory-item"]:has([data-test*="${productId}"])`);
  }
  ...
  private getProductNameLink(productId: string): Locator {
    ...
    return this.page.locator(`[data-test="inventory-item-name"]`).filter({ hasText: productId.replace(/-/g, ' ') }).first();
  }

  /**
   * Get product price for a specific product
   * @param productId - The product identifier
   */
  private getProductPrice(productId: string): Locator {
    return this.getProductContainer(productId).locator('[data-test="inventory-item-price"]');
  }
  ...
  async addProductToCart(product: ProductData): Promise<void> {
    ...
    // Validate button changed to "Remove"
  ...
  async removeProductFromCart(product: ProductData): Promise<void> {
    ...
    // Validate button changed back to "Add to cart"
    const addToCartButton = this.getAddToCartButton(product.id);
    await expect(addToCartButton).toBeVisible();
    await expect(addToCartButton).toHaveText('Add to cart');
  ...
  async validateProductInformation(product: ProductData): Promise<void> {
    ...
    const productNameElement = this.page.locator(`[data-test="inventory-item-name"]`).filter({ hasText: product.name });
   ...
   */
  async validateProductSorting(sortType: 'name-asc' | 'name-desc' | 'price-asc' | 'price-desc'): Promise<void> {
