...
export class CheckoutPage {
  ...
  private readonly summaryContainer = '[data-test="checkout-summary-container"]';
  ...
  async fillCheckoutInformation(customerData: {
    ...
    TestLogger.logAction(`Filling checkout information for ${customerData.firstName} ${customerData.lastName}`);
  ...
  async continueToOverview(): Promise<void> {
    ...
    TestLogger.logValidation('Successfully navigated to checkout overview', true);
  }
  ...
  async validateCheckoutOverview(): Promise<void> {
    ...
    TestLogger.logValidation('Checkout overview page validated successfully', true);
  }
  ...
  async getOrderSummary(): Promise<{
    ...
    TestLogger.logInfo(`Order summary - Item Total: $${itemTotal}, Tax: $${tax}, Total: $${total}`);
  ...
  async validateCheckoutError(expectedErrorMessage?: string): Promise<void> {
    ...
    if (expectedErrorMessage) {
      const errorText = await this.page.locator(this.errorContainer).textContent();
      expect(errorText).toContain(expectedErrorMessage);
      TestLogger.logValidation(`Error message validated: ${expectedErrorMessage}`, true);
    }
