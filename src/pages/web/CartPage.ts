...
export class CartPage {
	...
	async calculateTotalValue(): Promise<number> {
		...
		console.log(
			`ℹ️ Individual prices: [${prices.map((p) => `$${p.toFixed(2)}`).join(", ")}]`,
		);
	 ...
	 */
	async validateTotalAgainstData(
		...
		const expectedTotal = expectedProducts.reduce(
		...
		console.log(
			`ℹ️ Difference: $${Math.abs(actualTotal - expectedTotal).toFixed(2)}`,
		);
		console.log(
			`${isValid ? "✅" : "❌"} Total validation: ${isValid ? "PASSED" : "FAILED"}`,
		);
	...
	async validateCartContents(expectedProducts: ProductData[]): Promise<void> {
		console.log(
			`🔍 Validating cart contains ${expectedProducts.length} products`,
		);
