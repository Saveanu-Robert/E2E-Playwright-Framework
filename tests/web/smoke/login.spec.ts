...
import { INVALID_CREDENTIALS, SAUCEDEMO_USERS } from '@data/testdata/saucedemo.users';
...
test.describe('sauceDemo Smoke Test - Login', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    if (testInfo.status === 'failed') {
      ...
      await PageUtils.takeScreenshot(
        page,
        `smoke-failed-${testInfo.title.replace(/\s+/g, '-')}`,
        true,
      );
    }
  });
   ...
   */
  test('should enforce basic security measures', async ({ page, loginPage, baseUrl }) => {
