...
import { SAUCEDEMO_PRODUCTS, CHECKOUT_DATA } from '@data/testdata/saucedemo.products';
...
test.describe('SauceDemo E2E Test - Complete User Journey', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    if (testInfo.status === 'failed') {
      ...
      await PageUtils.takeScreenshot(page, `e2e-failed-${testInfo.title.replace(/\s+/g, '-')}`, true);
  ...
  test('should complete full customer purchase journey successfully', async ({ 
    ...
    const testDescription = 'End-to-end validation of complete customer shopping and purchase experience';
    ...
    try {
      ...
      for (const product of targetProducts) {
        TestLogger.logAction(`Customer examining product: ${product.name} (${product.priceDisplay})`);
      ...
      TestLogger.logInfo(`Customer shipping info: ${customerData.firstName} ${customerData.lastName}, ${customerData.postalCode}`);
      ...
      TestLogger.logError('E2E customer journey failed', error instanceof Error ? error.message : String(error));
  ...
  test('should provide consistent experience across browser environments', async ({ 
    ...
    try {
      ...
      TestLogger.logInfo(`Browser: ${browserName}, Viewport: ${viewportSize?.width}x${viewportSize?.height}`);
      ...
      const expectedMaxTime = browserName === 'webkit' ? 3000 : 2000; // WebKit can be slower
      ...
      TestLogger.logError(`Cross-browser E2E test failed in ${browserName}`, error instanceof Error ? error.message : String(error));
      TestLogger.logTestEnd(testName, false);
      throw error;
    }
  });
