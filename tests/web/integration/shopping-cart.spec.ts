...
import { AssertionUtils, PageUtils, TestLogger } from '@utils/helpers/test.utils';
...
test.describe('sauceDemo Integration Test - Shopping Cart Flow', () => {
  ...
  test('should integrate product sorting with cart operations', async ({
    ...
    try {
      ...
      const currentSort = page.locator('[data-test="product-sort-container"]');
      TestLogger.logInfo(
        `Sort state after navigation: ${currentSort} (SauceDemo resets to default 'az')`,
      );
      await expect(currentSort).toHaveValue('az'); // SauceDemo resets to default
