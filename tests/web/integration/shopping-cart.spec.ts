...
import { SAUCEDEMO_PRODUCTS, CHECKOUT_DATA, INVALID_CHECKOUT_DATA } from '@data/testdata/saucedemo.products';
import { TestLogger, PageUtils, AssertionUtils, DataUtils } from '@utils/helpers/test.utils';
...
test.describe('SauceDemo Integration Test - Shopping Cart Flow', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    if (testInfo.status === 'failed') {
      ...
      await PageUtils.takeScreenshot(page, `integration-failed-${testInfo.title.replace(/\s+/g, '-')}`, true);
    }
  });
  ...
  test('should integrate login, shopping, and checkout processes', async ({ 
    ...
    const testDescription = 'Test integration between login, shopping cart, and checkout systems';
    
    TestLogger.logTestStart(testName, testDescription);

    try {
      // Phase 1: Authentication Integration
      TestLogger.logStep(1, 'Test authentication integration with application access');
      ...
      TestLogger.logValidation('Authentication successfully integrated with inventory access', true);

      // Phase 2: Product Management Integration
      TestLogger.logStep(2, 'Test product selection integration with cart system');
      ...
      TestLogger.logValidation(`Product selection integrated with cart (${expectedCartCount} items)`, true);

      // Phase 3: Cart Management Integration
      TestLogger.logStep(3, 'Test cart management integration with product data');
      ...
      TestLogger.logValidation('Cart management integrated with product data correctly', true);

      // Phase 4: Checkout Process Integration
      TestLogger.logStep(4, 'Test checkout process integration with cart and forms');
      ...
      TestLogger.logValidation('Form validation integrated with checkout process', true);
      ...
      TestLogger.logValidation('Complete checkout integration validated successfully', true);
      ...
      TestLogger.logValidation('System state consistently integrated across all components', true);
      TestLogger.logTestEnd(testName, true);
      
    } catch (error) {
      TestLogger.logError('Integration test failed', error instanceof Error ? error.message : String(error));
  ...
  test('should integrate product sorting with cart operations', async ({ 
    ...
    const testDescription = 'Test integration between product sorting and cart operations';
    ...
    try {
      ...
      TestLogger.logInfo(`Initial product order: ${initialProducts.join(', ')}`);
      ...
      TestLogger.logValidation('Product sorting integrated with cart state', true);

      // Step 3: Test price sorting integration
      TestLogger.logStep(3, 'Test price sorting integration with cart operations');
      ...
      TestLogger.logValidation('Price sorting integrated with cart operations', true);
      ...
      const currentSort = await page.locator('[data-test="product-sort-container"]').inputValue();
      TestLogger.logInfo(`Sort state after navigation: ${currentSort} (SauceDemo resets to default 'az')`);
      expect(currentSort).toBe('az'); // SauceDemo resets to default
      
      TestLogger.logValidation('Navigation integrated with proper state management (sort reset is expected)', true);
      TestLogger.logTestEnd(testName, true);
      
    } catch (error) {
      TestLogger.logError('Product sorting integration test failed', error instanceof Error ? error.message : String(error));
      TestLogger.logTestEnd(testName, false);
      throw error;
    }
  });
